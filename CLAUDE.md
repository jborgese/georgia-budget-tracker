...
Current status: change detection, the RLGF county ETL (`pipeline/etl_rlgf.py`), the state-level ETL (`pipeline/etl_state.py`), and the statewide dashboard homepage are live; county pages and the choropleth are not built yet.
